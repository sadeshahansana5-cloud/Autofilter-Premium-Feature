...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1003598963172').split()]  # Channel id for auto indexing (make sure bot is admin)
...
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-5025355345')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1003819851778')  # Request channel id (make sure bot is admin)
...
auth_channels     = environ.get("AUTH_CHANNELS", "-1003661202269")# Channels for force sub (make sure bot is admin)
...
TMDB_API_KEY = environ.get('TMDB_API_KEY', 'd2d002918cb1dfef9148bbf4f1abdcdc') # preffer to use your own tmdb API Key get it from https://www.themoviedb.org/settings/api
...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/ultraprofilter_group') # Group link for the bot
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/ultraprofilter_group') # Owner link for the bot
UPDATE_CHNL_LNK = environ.get('UPDATE_CHNL_LNK', 'https://t.me/shprofilterupdate') # Update channel link for the bot
...
STREAM_MODE = (environ.get('STREAM_MODE', False)) # Set Stream mode True or False
PREMIUM_STREAM_MODE = (environ.get('PREMIUM_STREAM_MODE', False)) # Set Stream mode True or False only for premium users
